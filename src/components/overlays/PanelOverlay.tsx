      ...
      <div className="overlay-sheet" style={{ maxHeight: '92vh' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="overlay-body">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 20 }}>
            {[
              { key: 'demo', label: t('app.panel.demoBalance'), value: demoBalance, color: '#F59E0B', borderRgba: 'rgba(245,158,11,.3)', iconBg: 'rgba(245,158,11,.12)' },
              { key: 'real', label: t('app.panel.realBalance'), value: realBalance, color: 'var(--g0)', borderRgba: 'rgba(0,214,143,.3)', iconBg: 'rgba(0,214,143,.12)' },
            ].map(card => (
              <div key={card.key} style={{
                position: 'relative', overflow: 'hidden',
                background: 'var(--bg0)', border: `1px solid ${card.borderRgba}`,
                borderRadius: 'var(--r2)', padding: '14px 16px 16px',
              }}>
                {/* Decorative flowing wave lines in the accent color */}
                <svg
                  viewBox="0 0 160 100" preserveAspectRatio="none"
                  style={{ position: 'absolute', right: -6, bottom: -6, width: '75%', height: '68%', pointerEvents: 'none' }}
                >
                  <path d="M0 80 C 30 60, 40 90, 70 55 S 120 20, 160 45" stroke={card.color} strokeWidth="1.4" fill="none" opacity=".5" />
                  <path d="M0 92 C 35 75, 50 100, 85 68 S 130 35, 160 60" stroke={card.color} strokeWidth="1.4" fill="none" opacity=".3" />
                  <path d="M10 100 C 45 88, 60 108, 95 80 S 140 50, 160 72" stroke={card.color} strokeWidth="1.4" fill="none" opacity=".18" />
                </svg>
                <div style={{ position: 'relative', zIndex: 1 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: card.color, letterSpacing: .5, marginBottom: 4, textTransform: 'uppercase' }}>{card.label}</div>
                  <div style={{ fontSize: 18, fontWeight: 900, color: card.color, fontFamily: 'JetBrains Mono' }}>
                    ${card.value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
                  <div style={{
                    marginTop: 10, width: 30, height: 30, borderRadius: 8,
                    background: card.iconBg, color: card.color,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                  }}>
