...
export default function BottomControls() {
  ...
  return (
    <div className="bottom-area">
      ...
      {/* Investment / Time / Payout — merged into one bordered row with
          dividers, matching the reference design (was 3 separate boxes) */}
      <div className="bc-merged-row">
        <div className="bc-merged-col" onClick={() => setOverlay('expiry' as any)}>
          <div className="bc-merged-label">{t('app.trade.amount')}</div>
          <div className="bc-merged-val">${amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
        </div>
        <div className="bc-merged-divider" />
        <div className="bc-merged-col" onClick={() => setOverlay('expiry' as any)}>
          <div className="bc-merged-label">{t('app.trade.expiry')}</div>
          <div className="bc-merged-val">{expDisp}</div>
        </div>
        <div className="bc-merged-divider" />
        <div className="bc-merged-col">
          <div className="bc-merged-label">{t('app.trade.payout')}</div>
          <div className="bc-merged-val" style={{ color:'var(--g0)' }}>{payout}% <span style={{ color:'var(--g0)', fontSize:11 }}>+${profit}</span></div>
        </div>
      </div>
