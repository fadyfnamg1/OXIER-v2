...
// The always-visible rail mirrors the reference design's 5-icon left-hand
// column: pencil (opens the full tool list), line, T (closest existing
// tool — hline, used for price/text labels), sliders (color/width/style),
// heart (favorite this chart). Every tool from TOOL_GROUPS below still
// exists and still works — the rest are reachable via the pencil's popover.
const PRIMARY_TOOLS: Tool[] = [
  { id: 'trendline', labelKey: 'tool.trendLine', icon: '↗' },
  { id: 'hline',     labelKey: 'tool.hline',     icon: 'T' },
];
  ...
  return (
    <div className={isFullscreen ? 'chart-fullscreen' : undefined} style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      ...
      <div style={{ flex:1, position:'relative', minHeight:0 }}>
        ...
        {/* ── Left icon rail (icon-only, no labels) — always visible, 5
             icons matching the reference design: pencil (more tools),
             line, T (label/price line), sliders (settings), heart
             (favorite this chart) ─────────────────────────────────────── */}
        ...
        {/* ── "More tools" popover — every other tool from TOOL_GROUPS plus
             color/width/style + clear-all, reachable from the pencil button
             so the always-visible rail stays at 5 icons ────────────────── */}
        {toolsOpen && (
