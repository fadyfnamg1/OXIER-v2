...
import { subscribeKline, subscribeTrade } from '../lib/liveSocket';

// All timeframes in display order — the full 5s→1d range, all shown as
// pills in the scrollable toolbar row (see .tf-bar, which already scrolls
// horizontally so the tool cluster on the right never gets pushed off).
const TFS: TF[] = ['5s','10s','30s','1m','3m','5m','15m','30m','1h','2h','4h','6h','8h','12h','1d'];
...
// Persist drawings (trend lines, Fibonacci, etc.) across refreshes and app
// restarts instead of losing them the moment the page reloads — the tools
// stay on the chart exactly as the client left them.
const DRAWINGS_KEY = 'ox_chart_drawings';
...
// The always-visible rail is the on-chart toolbar: pencil (opens the full
// tool list), the 7 primary draw tools below, a clear-all action, sliders
// (color/width/style), heart (favorite this chart) — every icon here is
// icon-only (no label) to save space directly on the chart. Every other
// tool still exists and still works — reachable via the pencil's popover
// ("drawing tools") in TOOL_GROUPS.
...
export default function TradingChart() {
  ...
  // Holds the unsubscribe function for the backend-relayed kline stream
  // (see lib/liveSocket.ts) — replaces what used to be a raw WebSocket
  // connected straight to Binance.
  ...
  // High-frequency price smoothing: the Binance kline stream (and our own
  // short-TF REST polling) only pushes roughly once a second, which is why
  // the candle used to visibly "move, stop, move" while the price ticker
  // up top (fed by the much chattier @trade stream) looked continuous.
  // tradeWsRef subscribes to that same @trade stream (relayed through our
  // backend) and a rAF loop applies every tick straight to the forming
  // candle, so both move in lockstep. Holds an unsubscribe function, not a
  // raw WebSocket — see lib/liveSocket.ts.
  const tradeWsRef      = useRef<(() => void) | null>(null);
  ...
  // Cancels the previous loadData()'s in-flight Binance fetch(es) whenever a
  // new build starts (fast market/timeframe switching) — avoids racing
  // requests, wasted bandwidth, and unnecessary pressure on rate limits.
  const loadAbortRef     = useRef<AbortController | null>(null);
  // True whenever barsRef.current currently holds genSimData() fallback
  // bars instead of real Binance history (fetchBinanceKlines failed on
  // load). Checked by connectWS()'s first real kline_update so it can
  // discard the fake bars and reload real history instead of layering a
  // real price on top of a fake one — which is what produced the sharp
  // vertical jump on reconnect.
  const usingSimDataRef  = useRef(false);

  const [lwcReady, setLwcReady]       = useState(false);
  // Flips true right after mainSeriesRef.current is actually assigned inside
  // buildChart. Needed because the open-trade price-line effect below only
  // reruns when its own deps change — on a page refresh the boot-time trade
  // sync in store.ts populates `trades` before this chart has finished
  // building its series, so that effect fires once, finds no series yet,
  // and never gets another chance until `trades` changes again (e.g.
  // opening a new trade). Depending on this too gives it that chance.
  ...
  const [drawings, setDrawings]       = useState<Drawing[]>(() => {
    try { return JSON.parse(localStorage.getItem(DRAWINGS_KEY) || '[]'); } catch { return []; }
  });
  ...
  const [chartFavorited, setChartFavorited] = useState(false);
  ...
  // The chart canvas now matches the app-wide dark navy surface instead of
  // the old white "Quotex-style" background, so it blends with the rest of
  // the platform (toolbars, overlays, bottom bar) instead of clashing.
  const bg          = '#070910';
  const gridColor   = 'rgba(160,190,255,.06)';
  const borderColor = 'rgba(160,190,255,.12)';
  const textColor   = 'rgba(255,250,244,.55)';

  // Keep drawingsRef in sync
  useEffect(() => { drawingsRef.current = drawings; }, [drawings]);

  // Save every change immediately — drawings must survive a refresh or the
  // app being reopened later, not just live in memory for the session.
  useEffect(() => {
    try { localStorage.setItem(DRAWINGS_KEY, JSON.stringify(drawings)); } catch { /* storage full/unavailable — ignore */ }
  }, [drawings]);
  ...
  function _dot(ctx: CanvasRenderingContext2D, p: {x:number;y:number}, color: string) {
    ...
    ctx.strokeStyle = 'rgba(20,24,32,.35)'; ctx.lineWidth = 1;
  ...
  const buildChart = useCallback(() => {
    ...
    if (tradeWsRef.current) { tradeWsRef.current(); tradeWsRef.current = null; }
    if (smoothingRafRef.current) { cancelAnimationFrame(smoothingRafRef.current); smoothingRafRef.current = null; }
  ...
  async function loadData(chart: any, mainSeries: any, hasSubInd: boolean, buildId: number) {
    ...
    // Cancel whatever previous loadData() call is still in flight so a fast
    // market/timeframe switch doesn't leave two requests racing each other.
    ...
    try {
      if (isShortTF) {
        ...
        const oneMBars = await fetchBinanceKlines(currentMarket.symbol, '1m', 1000, controller.signal);
        const tfSec = TIMEFRAMES[currentTF].sec;
        bars = genShortTFData(oneMBars, tfSec, currentMarket.symbol);
      } else {
        bars = await fetchBinanceKlines(currentMarket.symbol, TIMEFRAMES[currentTF].binance, 1000, controller.signal);
      }
      usingSimDataRef.current = false; // real history loaded successfully
    } catch (err: any) {
      // Intentional cancellation (a newer build superseded this one) — bail
      // out quietly. The newer build's own loadData() call owns the chart
      // now, and generating 2000 sim candles here would be wasted work.
      if (err?.name === 'AbortError') return;
      bars = genSimData(currentMarket.symbol, currentMarket.price, currentTF, 2000);
      usingSimDataRef.current = true;
    }
    ...
    connectTradeTicker(buildId);
    startSmoothingLoop(mainSeries, buildId);
  ...
  // ─── High-frequency price smoothing ───────────────────────────────────────
  // Binance's kline stream (and our own short-TF REST polling) only deliver
  // an update roughly once a second, which is what made the candle look like
  // it moved in steps. The @trade stream fires on every individual executed
  // trade — the same firehose the price ticker up top already feels smooth
  // from — so we tap it here too and let a rAF loop paint every tick
  // straight onto the forming candle.
  function connectTradeTicker(buildId: number) {
    if (!currentMarket) return;
    const symbol = currentMarket.symbol;
    // Relayed through our backend instead of connecting straight to
    // Binance's @trade stream — see lib/liveSocket.ts.
    const unsubscribe = subscribeTrade(symbol, (price) => {
      if (buildId !== buildIdRef.current) return;
      latestTickRef.current = price;
    });
    tradeWsRef.current = unsubscribe;
  }

  function startSmoothingLoop(mainSeries: any, buildId: number) {
    let lastApplied: number | null = null;
    function frame() {
      if (buildId !== buildIdRef.current) return; // stale build — loop dies here
      const p = latestTickRef.current;
      if (p !== null && p !== lastApplied) {
        lastApplied = p;
        const bars = barsRef.current;
        ...
        if (last) {
          ...
          const upd = (chartType === 'Candles' || chartType === 'HeikinAshi' || chartType === 'Bars')
            ? last : { time: last.time, value: last.close };
          try { mainSeries.update(upd); } catch {}
          setLivePrice(p);
        }
      }
      smoothingRafRef.current = requestAnimationFrame(frame);
    }
    smoothingRafRef.current = requestAnimationFrame(frame);
  }
  ...
  function connectShortTFTimer(mainSeries: any, chart: any, buildId: number) {
    ...
    // Fetch real price through our backend proxy instead of Binance directly
    let currentPrice = lastBar?.close || currentMarket.price;
    const fetchPrice = async () => {
      try {
        const r = await fetch(`${BACKEND}/api/market-data/ticker-price?symbol=${currentMarket.symbol}`);
        if (r.ok) { const d = await r.json(); currentPrice = d.price; }
      } catch {}
    };
    fetchPrice();
    ...
    shortTFTimerRef.current = setInterval(async () => {
      ...
      // Prefer the live @trade tick (updates continuously via the
      // smoothing loop already) — REST polling is now only a fallback for
      // the brief window before that socket delivers its first tick.
      if (latestTickRef.current !== null) {
        currentPrice = latestTickRef.current;
  ...
  // ─── Live candle updates for normal timeframes ────────────────────────────
  // Relayed through our own backend (see lib/liveSocket.ts + the server's
  // binance-relay.service.ts) instead of connecting straight to Binance's
  // @kline stream. Many users watching the same symbol/timeframe now share
  // one upstream connection on the server instead of opening one each.
  function connectWS(mainSeries: any, chart: any, buildId: number) {
    if (!currentMarket) return;
    const symbol = currentMarket.symbol;
    const interval = TIMEFRAMES[currentTF].binance;
    let lastMsgAt = Date.now();

    function subscribe() {
      if (buildId !== buildIdRef.current) return () => {};
      return subscribeKline(symbol, interval, (bar) => {
        if (buildId !== buildIdRef.current) return;
        lastMsgAt = Date.now();

        // We were showing genSimData() fallback bars (real history fetch
        // had failed) and this is the first real tick since then. Don't
        // layer a real price onto a fake candle — that's exactly what
        // produced the sharp vertical jump on reconnect. Instead, throw
        // away the fake bars and do a full reload of real history; this
        // single incoming bar will arrive again once the fresh subscribe
        // completes, so it's safe to skip applying it here.
        if (usingSimDataRef.current) {
          usingSimDataRef.current = false;
          buildChart();
          return;
        }
    ...
    // Watchdog: if no kline update arrives for 8s (e.g. a flaky mobile
    // connection, or the shared socket silently dropped), re-subscribe to
    // force a fresh one instead of leaving the chart (and live price)
    // silently frozen.
  ...
  // ─── Render ────────────────────────────────────────────────────────────────
  return (
    <div className={isFullscreen ? 'chart-fullscreen' : undefined} style={{ display:'flex', flexDirection:'column', height:'100%' }}>

      {/* ── Timeframe + Chart-type bar ─────────────────────────────────────── */}
      <div className="tf-bar-wrap">
        {/* Scrollable timeframe pills — this row alone scrolls, so the tool
            buttons on the right never get pushed off-screen. */}
        ...
        {/* Fixed, always-visible tool cluster — never scrolls out of view */}
        <div className="tf-bar-tools">
          <div style={{ position:'relative' }}>
            <button
              title="Chart type"
              className={`tf-btn ct-icon-btn ${chartTypeMenuOpen ? 'active' : ''}`}
              onClick={() => setChartTypeMenuOpen(o => !o)}
            >{CHART_TYPE_ICONS[chartType]}</button>
            {chartTypeMenuOpen && (
              <div className="chart-type-menu">
                {CHART_TYPES.map(ct => (
                  <button
                    key={ct}
                    className={`chart-type-menu-item ${chartType === ct ? 'active' : ''}`}
                    onClick={() => { setChartType(ct); setChartTypeMenuOpen(false); }}
                  >
                    <span className="chart-type-menu-ico">{CHART_TYPE_ICONS[ct]}</span>
                    {ct}
                  </button>
                ))}
              </div>
      ...
      <div style={{ flex:1, position:'relative', minHeight:0 }}>
        <div
          ref={containerRef}
          style={{ width:'100%', height: hasSubChart ? 'calc(100% - 65px)' : '100%', position:'relative' }}
        >
        ...
        {hasSubChart && subInds.length > 0 && (
          <div style={{ position:'absolute', bottom:0, left:0, right:0, height:65, display:'flex', flexDirection:'column' }}>
            <div style={{
        ...
        {/* ── Left icon rail (icon-only, no labels) — always visible on the
             chart: pencil (opens drawing tools), the 7 primary draw tools
             (trend line, ray, h-ray, h-line, v-line, arrow, fib extension),
             clear-all, sliders (settings), heart (favorite this chart) ── */}
        <div className="chart-tool-rail">
          <button
            title={toolsOpen ? 'Close more tools' : 'More drawing tools'}
            className={`chart-tool-btn ${toolsOpen ? 'active' : ''}`}
          ...
          <div className="chart-tool-rail-scroll">
            {PRIMARY_TOOLS.map(({ id, labelKey, icon }) => {
              ...
              return (
                <button
                  key={labelKey}
                  title={t(labelKey)}
                  onClick={() => { if (id === null) setDrawMode(null); else setDrawMode(d => d === id ? null : id); }}
                  className={`chart-tool-btn ${isActive ? 'active' : ''}`}
                  style={isActive ? { color, borderColor: color, background: `${color}18` } : undefined}
                >
                  {icon}
                </button>
              );
          ...
          {/* Clear all — icon-only, disabled (not hidden) when there's
              nothing to clear, so the rail's width stays stable. */}
          <button
            title={drawings.length > 0 ? `Clear all (${drawings.length})` : 'Clear all'}
          ...
          >
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2m3 0-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg>
          </button>

          <button
            title="Settings"
            className={`chart-tool-btn ${settingsOpen ? 'active' : ''}`}
          ...
          <button
            title={chartFavorited ? 'Remove from favorite charts' : 'Favorite this chart'}
            className={`chart-tool-btn ${chartFavorited ? 'active' : ''}`}
            onClick={() => setChartFavorited(f => !f)}
            style={chartFavorited ? { color:'#F43F5E', borderColor:'rgba(244,63,94,.4)', background:'rgba(244,63,94,.1)' } : undefined}
          >
            <svg width="15" height="15" viewBox="0 0 24 24" fill={chartFavorited ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>
          </button>
        </div>

        {/* ── "More tools" popover (drawing tools) — every other tool from
             TOOL_GROUPS plus color/width/style, reachable from the pencil
             button. Clear-all now lives on the always-visible rail ──────── */}
        {toolsOpen && (
          <div className="chart-tool-more">
            <div className="chart-tool-more-scroll">
              {/* Cursor (deselect) — standalone, not a "group" of its own */}
              <button
                onClick={() => setDrawMode(null)}
                className={`chart-tool-more-item ${drawMode === null ? 'active' : ''}`}
                style={drawMode === null ? { color: 'var(--g0)' } : undefined}
              >
                <span className="chart-type-menu-ico">↖</span>
                {t('tool.cursor')}
              </button>
              {TOOL_GROUPS.map(group => (
                <div key={group.labelKey}>
                  <div className="chart-tool-more-group-label">{t(group.labelKey)}</div>
                  {group.tools.map(({ id, labelKey, icon }) => {
                    ...
                    return (
                      <button
                        key={labelKey}
                        onClick={() => { if (id === null) setDrawMode(null); else setDrawMode(d => d === id ? null : id); }}
                        className={`chart-tool-more-item ${isActive ? 'active' : ''}`}
                        style={isActive ? { color } : undefined}
                      >
                        <span className="chart-type-menu-ico">{icon}</span>
                        {t(labelKey)}
                      </button>
                    );
                  })}
            ...
            {/* Settings (color / width / style) toggle */}
            <button
              title="Settings"
              className={`chart-tool-btn ${settingsOpen ? 'active' : ''}`}
              onClick={() => setSettingsOpen(o => !o)}
            ...
            >
              <span style={{ width:12, height:12, borderRadius:4, background:drawColor, display:'inline-block' }} />
            </button>
          </div>
        )}

        {/* ── Compact settings popover (color / width / style) ───────────── */}
        {toolsOpen && settingsOpen && (
          <div className="chart-tool-settings">
            <div className="chart-tool-settings-row">
              {DEFAULT_COLORS.map(c => (
                <div
                  key={c}
                  onClick={() => setDrawColor(c)}
                  style={{
                    width:16, height:16, borderRadius:5, background:c, cursor:'pointer',
                    border: drawColor === c ? '2px solid #fff' : '2px solid transparent',
                  }}
                />
              ))}
            </div>
            <div className="chart-tool-settings-row" style={{ alignItems:'center' }}>
              <input type="range" min={1} max={5} step={0.5} value={drawWidth}
                onChange={e => setDrawWidth(parseFloat(e.target.value))}
                style={{ flex:1, accentColor:'var(--g0)' }} />
              <span style={{ fontSize:10, color:'var(--t3)', fontWeight:700, minWidth:16, textAlign:'right' }}>{drawWidth}</span>
            </div>
            <div className="chart-tool-settings-row">
              {(['solid','dashed','dotted'] as const).map(s => (
                <button
                  ...
                  className={`chart-style-btn ${drawStyle === s ? 'active' : ''}`}
        ...
        {editDraw && (
          <div style={{
            ...
            background:'var(--bg1)', border:'1px solid var(--border2)', borderRadius:18,
              ...
              <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
                {DEFAULT_COLORS.map(c => (
                  <div
                    ...
                    style={{ width:22, height:22, borderRadius:8, background:c, cursor:'pointer', border: editDraw.color === c ? '2px solid #fff' : '2px solid transparent', transition:'all .18s var(--ease)' }}
                  />
                ))}
              </div>
            </div>
            ...
            <div style={{ display:'flex', gap:6 }}>
              <button
                ...
                style={{ flex:1, padding:'9px 0', borderRadius:10, border:'1px solid rgba(255,61,87,.3)', background:'rgba(255,61,87,.08)', color:'var(--red)', fontWeight:700, cursor:'pointer', fontSize:12, fontFamily:'inherit', transition:'all .2s var(--ease)' }}
              >Delete</button>
              <button
                onClick={() => setEditDraw(null)}
                style={{ flex:1, padding:'9px 0', borderRadius:10, border:'1px solid var(--border2)', background:'var(--bg2)', color:'var(--t1)', fontWeight:700, cursor:'pointer', fontSize:12, fontFamily:'inherit', transition:'all .2s var(--ease)' }}
              >Close</button>
            </div>
          </div>
        )}
