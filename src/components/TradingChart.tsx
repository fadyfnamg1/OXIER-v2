...
// The always-visible rail shows only these 6 (cursor + 5 core drawing
// tools), matching the reference design's compact left-hand icon column.
// Every other tool from TOOL_GROUPS below still exists and still works —
// it's just reachable via the "more tools" popover, not cluttering the rail.
  ...
  return (
    <div className={isFullscreen ? 'chart-fullscreen' : undefined} style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      ...
      <div className="tf-bar-wrap">
        ...
        <div className="tf-bar-tools">
          ...
          <button
            ...
            className={`tf-btn ct-icon-btn ${toolsOpen ? 'active' : ''}`}
      ...
      <div style={{ flex:1, position:'relative', minHeight:0 }}>
        ...
        {/* ── Drawing tools icon rail (icon-only, no labels) — always
             visible, 6 core tools, matching the OXIER reference design ── */}
        <div className="chart-tool-rail">
          ...
          {/* Clear all — always visible, matching the reference; a no-op
              (nothing to clear) when there are no drawings yet */}
          <button
            title={drawings.length ? `Clear all (${drawings.length})` : 'Clear all'}
            ...
            style={{ borderTop:'1px solid var(--border)', opacity: drawings.length ? 1 : .4 }}
          >
        ...
        {/* ── "More tools" popover — every other tool from TOOL_GROUPS plus
             color/width/style, reachable from the pencil button in the
             tf-bar so the always-visible rail stays at 6 icons ────────── */}
        {toolsOpen && (
